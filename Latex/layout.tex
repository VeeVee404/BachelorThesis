...
\begin{titlepage}
    \centering
    {\Large University of Salzburg\par}
    \vspace{1cm}
...
Readability is the metric, whether the code is understandable in terms of intent, logic and rationale. This is to avoid any confusion or ambiguity when it comes to debugging or editing the code in any way. The most important aspect to ensure a code is readable, is clear names of functions and variables, so it is clear what their purpose is.

\subsection{Structure}
Code structure refers to the organization and arrangement of code elements such as functions, classes, modules, and control flow within a software system. A well-defined code structure emphasizes modularity, readability, consistency, and maintainability, enabling easier debugging, testing, and future extension of the application.

\subsection{Idiomatic Use}
Idiomatic use describes how closely the code follows the existing conventions of a given programming language. This includes language specific features, structures and design principles.

\subsection{Unnecessary Complexity}
Unnecessary complexity looks at any redundancy, overly complicated logic or implementations that are difficult to understand within the code. This can also include excessive use of abstraction or patterns where they would not be necessary.
...
Output format incorrect & incorrect \\
\bottomrule
\end{tabular}
\end{table}
...
All experiments are executed on the same hardware and software environment. 
\begin{itemize}
    \item CPU: AMD Ryzen 3 4300GE
    \item RAM: 16GB
    \item Operating system: Virtual Ubuntu environment on Windows 11 
    \item Python version: Python (v3.14.2)
    \item Rust version: Rust (v1.94.1)
    \item Development environment: Visual Studio Code (v1.115.0)
\end{itemize}
...
|-- thesis/
\end{lstlisting}
Each year directory contains the \texttt{problems}, \texttt{solutions}, and \texttt{benchmarking-test}. For every problem statement there is a part A and a part B, except for the very last problem, since the last problem only has one part to it. Therefore the final number of results is 88.
...
The code evaluations were conducted with the method 'LLM-as-a-judge', with a standardized prompt and the LLM-Model Codex by OpenAI. In additin to this method of evaluation, the two code evaluation systems 'SonarQube' and 'Codeacy' were used, but these systems did not provide an evaluation for all Codes.
...
Furthermore, the code structure was evaluated based on readability, idiomatic use of the languages, overall structure and any unneccesary complexity within the code.

\section{Correctness}
The generated code was functionally correct on the first attempt in all tested cases. No additional debugging or structural modifications were required and the repair prompt was not used.
...
\section{Idiomatic Use}

\section{Unnecessary Complexity}
